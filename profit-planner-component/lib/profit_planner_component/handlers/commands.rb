...
module ProfitPlannerComponent
  module Handlers
    class Commands
      ...
      #   profit_planner_id = do_something.profit_planner_id

      #   profit_planner, version = store.fetch(profit_planner_id, include: :version)

      #   if profit_planner.something_happened?
      #     logger.info(tag: :ignored) { "Command ignored (Command: #{do_something.message_type}, ProfitPlanner ID: #{profit_planner_id})" }
      #     return
      #   end
      ...
      #   stream_name = stream_name(profit_planner_id)
